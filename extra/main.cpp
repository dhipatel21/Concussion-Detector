...
//    HAL_UART_Receive(&huart4, end_calib, sizeof(end_calib), 100);
    ...
    if (skip) {
        LCD_writePixels(&hspi1, LCD_color565(255,255,255), 40, 240, 100, 250); // initialize to all white screen
    ...
    LCD_writePixels(&hspi1, LCD_color565(255,255,255), 30, 225, LCD_WIDTH, 280); // initialize to all white screen, except for running message
    skip = 0;
    LCD_drawString(&hspi1, 65, 110, highest, sizeof(highest), black, 4);        // display "highest acceleration so far: ____ g"
    LCD_drawString(&hspi1, 17, 150, highest1, sizeof(highest1), black, 4);
    LCD_drawString(&hspi1, 65, 190, highest2, sizeof(highest2), black, 4);
    
    // TODO: add interrupt callback function, determine how big buffer needs to be (receiving data every second), post-analysis of data?
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
        ...
        HAL_UART_Receive_IT(&huart4, read_buf, sizeof(read_buf));
        ...
        if (skip) {
            if (max < 58.0) {
                LCD_drawString(&hspi1, 80, 260, buffer, sizeof(buffer), green, 5);
                LCD_drawChar(&hspi1, 180, 260, g, green, 5);
            }
        }

        if (accel >= 58.0) {    // check if threshold is met/exceeded; using 58 to account for rounding error
            button = 0;
            ...
            if (skip) {
                if (max >= 58.0) {
                    LCD_drawString(&hspi1, 80, 260, buffer, sizeof(buffer), red, 5);
                    LCD_drawChar(&hspi1, 180, 260, g, red, 5);
                }
            }
            LCD_drawString(&hspi1, 50, 0, concussion_message3, sizeof(concussion_message3), red, 4);
            LCD_drawString(&hspi1, 50, 30, concussion_message4, sizeof(concussion_message4), red, 4);
            ...
            LCD_writePixels(&hspi1, LCD_color565(255,255,255), 80, 180, LCD_WIDTH, 290); // initialize to all white screen
        }
        skip = 0;
        max = 0.0;
    }
